...
int init_atm_list() {
    ...
    int line_size = FILE_LINE_SIZE(cur_meta) + 1;
    line_size = FILE_LINE_SIZE(old_meta) + 1 > line_size ? FILE_LINE_SIZE(old_meta) + 1 : line_size;
    char format[100], line[line_size], balance_buffer[50];
    fscanf(file, "%[^\n]\n", line);
    sprintf(format, "%s%c%%%d[^'%c']%c%s%c%%%ds%c%s%c%%%ds%c%s%c%%%d[^'%c']%c\n",
        old_meta.tags[0], old_meta.separator, old_meta.data_sizes[0], old_meta.separator, old_meta.separator,
        old_meta.tags[1], old_meta.separator, old_meta.data_sizes[1], old_meta.separator,
        old_meta.tags[2], old_meta.separator, old_meta.data_sizes[2], old_meta.separator,
        old_meta.tags[3], old_meta.separator, old_meta.data_sizes[3], old_meta.separator, old_meta.separator
    );
...
void atm_to_file(ATM *atm) {
    ...
    fprintf(file,
        ...
        cur_meta.tags[0], cur_meta.separator, cur_meta.data_sizes[0], atm->name, cur_meta.separator,
        cur_meta.tags[1], cur_meta.separator, cur_meta.data_sizes[1], atm->account, cur_meta.separator,
        cur_meta.tags[2], cur_meta.separator, cur_meta.data_sizes[2], atm->pin, cur_meta.separator,
        cur_meta.tags[3], cur_meta.separator, cur_meta.data_sizes[3], atm->balance, cur_meta.separator
    );
...
void update_atm_file(int atm_index, int data_type, char *modified_data) {
    ...
    char line[FILE_LINE_SIZE(cur_meta) + 1], *tag_ptr;
    fscanf(file, "%[^\n]", line);

    if((tag_ptr = strstr(line, cur_meta.tags[data_type])) != NULL) {
        fseek(file, - strlen(line) + (tag_ptr - line) + strlen(cur_meta.tags[data_type]) + 1, SEEK_CUR);
        fprintf(file, "%-*s", cur_meta.data_sizes[data_type], modified_data);
    }
...
int pin_input(char *input, int *input_size, char *ch, char *pin_to_check, int is_censored) {
    *ch = 0;
    while(1) {
        if((*input_size = unbuffered_input(input, cur_meta.data_sizes[2], 1, 1, is_censored, *ch)) == OP_CANCELLED) return OP_CANCELLED;
        if(*input_size == cur_meta.data_sizes[2]) {
...
void receipt(long long int withdraw_amount) {
    ...
    pad = (UI_WIDTH + strlen(cur_meta.bank_name)) / 2;
    sprintf(line_write, "%*s%*s\n", pad, cur_meta.bank_name, UI_WIDTH - pad, "");
    strcat(result_str, line_write);
    ...
    memset(buffer2 + 4, '*', cur_meta.data_sizes[1] - 8);
    sprintf(line_write, "%s%*s \n", buffer, UI_WIDTH - strlen(buffer) - 1, buffer2);
    strcat(result_str, line_write);
    ...
    sprintf(line_write, "%s%*s \n", buffer, UI_WIDTH - strlen(buffer) - 1, "Rut tien mat tai AMT");
    strcat(result_str, line_write);
